// 파일이름	:	5_CAN_Transmit_Receive.c
// 대상장치	:	TR2837xD 트레이닝 키트
// 파일버전	:	1.00
// 갱신이력	:	2024-07-19, 버전 1.00
// 예제설명	:

//************************************************************************************************************************************************************************
//
// 본 예제는 TMS320F2837xD(F28377D or F28379D) 칩이 실장된 TR2837xD 트레이닝 키트를 대상으로 하고 있으며,
// TMS320F2837xD 칩의 CAN(DCAN) 모듈과 트레이닝 키트의 CAN 통신 트랜시버(TCAN1044V)를 이용한 테스트 데이터 송수신을 구현하고 있습니다.
//
// TR2837xD 트레이닝 키트의 '(5) COMM.' 회로 블록에는 TI사의 CAN-FD 통신 트랜시버 - TCAN1044V가 1개 탑재되어 있으며,
// 이를 이용하면 다양한 CAN 통신 기반 시스템과의 연결을 구성할 수 있습니다.
//
// * CAN_TX : GPIO70 / CANA_RX
// * CAN_RX : GPIO71 / CANA_TX
//
// 본 예제는 전처리 구문 'TRANSMIT' 이 정의된 경우, F2837xD 칩 DCAN 컨트롤러의 32개 메시지 오브젝트들 중 1번 메시지 오브젝트를
// TX로 운용하여 CAN 테스트 데이터 송신장치로 동작하며, 해당 전처리 구문이 정의되어 있지 않을 때, 메시지 오브젝트 1번을 RX로 운용하여
// CAN 테스트 데이터 수신장치로 동작합니다. (테스트 데이터 송수신 횟수 : 8회)
//
//
//************************************************************************************************************************************************************************
